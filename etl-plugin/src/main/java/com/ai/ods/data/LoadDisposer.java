...
public class LoadDisposer{
	
	private static Logger logger = LoggerFactory.getLogger(LoadDisposer.class);
	...
	private void disposeFlowFile(File f) throws Exception{
		...
		if(!StringUtils.isEmpty(fdp.getdB2Statement())){
			...
			cl = cl.replaceFirst("%DATE%", tableType == 0 ? TimeUtils.date2String(fileDate, "yyyyMMdd") : TimeUtils.date2String(fileDate, "yyyyMM"));
			//拼装命令
		...
		//拼装命令
		String[] cmds = {"sh", sysp.getProperty("gisShell")};
		result = RunCommand.runProcess(cmds, sysp.getProperty("workDir"));
		logger.info("GIS_SHELL调用,处理结果:{}",result);	
